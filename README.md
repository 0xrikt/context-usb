...
### 1. 复制粘贴
最简单的方式：一键复制个人上下文，粘贴到 AI 对话开头。

### 2. MCP Server
支持 MCP 协议的 AI 工具（如 Claude Desktop、Cursor）可以直接读取你的上下文，无需手动粘贴。

在 Context USB 的「输出到 AI」页面启用 MCP Server，将生成的配置复制到 `claude_desktop_config.json`：

```json
{
  "mcpServers": {
    "context-usb": {
      "url": "https://context-usb.vercel.app/api/mcp?token=YOUR_TOKEN"
    }
  }
}
```
...
### 3. Chrome 浏览器扩展
安装 Chrome 扩展后，可以一键将上下文注入 AI 对话输入框。

支持：ChatGPT、DeepSeek、Kimi、豆包

安装方式：
1. 打开 `chrome://extensions`
2. 开启「开发者模式」
3. 点击「加载已解压的扩展程序」
4. 选择 `chrome-extension/build/` 目录
...
- MCP Token 是自包含的（上下文编码在 URL 中），服务器不存储任何数据
- 可随时一键清除所有数据
...
npm install
npm run build
# Load chrome-extension/build/ as unpacked extension in Chrome
```
